...
read -p -r "agora, me diga a linha que tu descomentou, só coloca a região, tipo 'pt_BR' ou 'en_US' e SIM, preciso das duas letras maíusculas no final. " lingua
while ! grep -q "^$lingua.UTF-8" /mnt/etc/locale.gen; do
  echo "Locale '$lingua' não encontrado no locale.gen"
  read -p -r "Tenta de novo, com algo tipo pt_BR ou en_US: " lingua
done
